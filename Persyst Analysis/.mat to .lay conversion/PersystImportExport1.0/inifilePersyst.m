function varargout = inifilePersyst(varargin)
%inifilePersyst Creates, reads, or writes data from/to a standard ini (ascii)
...
%
%       INIFILE(fileName,'write',keys,<style>,<sorted>)
%           Writes keys given as cell array of strings (see description of
%           the keys below).
%           Optional style variable: 'tabbed' writes sections,
...
%           Optional sorted variable: 'off' omits sorting (beware of
%           duplicates). By default, sorting is 'on'.
%
...
%----------------
%
% [v.1.4.3] 30.11.2018 (Maarten Schrooten)
% - NEW: Ability to avoid sorting (beware of duplicates)
% - NEW: Do not print '=' if value is empty, unless it is NaN
% - NEW: Print '=' instead of ' = ' for key/value pairs
% - NEW: Emty line before each section
% - NEW: Support for datetime variables in n2s
%
% [v.1.4.2] 12.01.2007
...
elseif (strcmpi(operation,'write'))
    ...
    if nargin < 5 || isempty(varargin{5})
        ...
        sorted = varargin{5};
        if ~(strcmpi(sorted, 'on') | strcmpi(sorted, 'off')) | ~ischar(sorted)
            error('Unsupported sorted value given or sorted value not given as a string');
        end
    end
    [m,n] = size(keys);
    if n < 4
...
elseif (strcmpi(operation,'write'))
    ...
    writekeys(fileName,keys,style,sorted);
    return
...
%------------------------------------
function writekeys(fileName,keys,style,sorted)
        ...
        if ~keysExist(ii) 
            if ~secsExist(ii) && ~isempty(keysIn(ii,1))
                if ~isempty(keysIn{ii,1})
                    datain = [datain sprintf(['\n%s' NL_CHAR],['[' keysIn{ii,1} ']'])];
                end                
