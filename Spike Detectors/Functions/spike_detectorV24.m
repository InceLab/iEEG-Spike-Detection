function [out, discharges, d_decim, envelope, background, envelope_pdf]=spike_detectorV24(d,fs,settings)

% V3: parallel processing is desabled
...
% settings defaults:------------------------------------------------------
global bandwidth; bandwidth=[10 80]; % [(-fl) (-fh)]
global k1; k1=3.65; % (-k1)%change to 2
global k2; k2=k1; % (-k2)
% global k2; k2=3; % (-k2)
global k3; k3=0; % (-k3)
global buffering; buffering=300; % (-buf)
global main_hum_freq; main_hum_freq=60; % (-h)
...
global discharge_tol; discharge_tol=0.02; % (-dt)
...
if ~exist('winsize'); winsize= floor(1*fs); end % (-w) def 5*fs
if ~exist('noverlap');noverlap= floor(0.5*fs); end % (-n) def 4*fs

if buffering/(winsize/fs)<10; buffering=(winsize/fs)*10; end%disp(['buffer size increased to ' num2str(buffering) ' sec.']); end
if buffering>(dataSize(1)/fs); buffering=dataSize(1)/fs; end%disp(['buffer size decreased to ' num2str(buffering) ' sec.']); end
...
if RFactor>1 || decimation~=fs
    ...
    for i=1:decItterations
        ...
        for ch=1:size(d,2)%changed form parfor to for
            fs = round(fs); %% Amir
...
for ch=1:size(d,2) % changed from parfor to for
